...
class Keyword(Plugin):
    def __init__(self):
        ...
        try:
            ...
            logger.warn(f"[keyword] config_path 【{config_path}】")

            logger.info("[keyword] {}".format(self.keyword))
            self.handlers[Event.ON_HANDLE_CONTEXT] = self.on_handle_context
            logger.info("[keyword] inited.")
        except Exception as e:
            logger.warn("[keyword] init failed, ignore or see https://github.com/gogooing/future-on-wechat/tree/master/plugins/keyword .")
            raise e
